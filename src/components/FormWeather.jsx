 ...
 const {
  ...
  const consultarAPI = ()=>{
    // https://api.openweathermap.org/data/2.5/weather?q=San%20Miguel%20de%20Tucum%C3%A1n,AR&appid={APIKEY}&units=metric&lang=es
     const response = fetch(`https://api.openweathermap.org/data/2.5/weather?q=San%20Miguel%20de%20Tucum%C3%A1n,AR&appid=${api}&units=metric&lang=es`)
     console.log(response)
  }

  return (
    <>
      <form className="flex flex-col md:flex-row gap-4 justify-between items-center mt-8" onSubmit={handleSubmit(buscarClima)}>
